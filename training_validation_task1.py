# -*- coding: utf-8 -*-
"""
Created on Tue Mar 14 09:28:31 2023
...
############################################# load sample data #######################################
with h5py.File("D:\\reconsrtcionMRI\\task1\\task1\\simulated_ground_truths.h5") as hf:
    gt_fids = hf["ground_truth_fids"][()] ### 200*2048*2  GT
    ppm = hf["ppm"][()]     #### 200*2048
    t = hf["t"][()]         ##### 200x2048
##############################################################################################
# transforming into frequency domain to use as the target data
gt_spec = np.fft.fftshift(np.fft.ifft(gt_fids,axis=1),axes=1) ##200,2048,2
...
x_train = x[:int(x.shape[0]*0.8)] #160*2048*40*1
x_test = x[int(x.shape[0]*0.8):]

y_train = y[:int(y.shape[0]*0.8)] ## 160*2048
y_test = y[int(y.shape[0]*0.8):]  ### 40*2048

ppm_train = ppm[:int(ppm.shape[0]*0.8)] ##160*2048
ppm_test = ppm[int(ppm.shape[0]*0.8):]  #40*2048
#print(x_train.shape)
#print(x_test.shape)
...
######################################################### Training Loop################################################
# We'll instantiate the necessary objects for a training loop and 
# perfom the optimization of our model's weights

################################################ dataset loader and dataset object ######################
...
################### hyber loss function ##################
#epoch 99/100 - loss: 0.00012582112054632385
#epoch 100/100 - loss: 0.00012561009422170356
#dy_tukey_loss
#epoch 100/100 - loss: 0.00011790076809436698
#### huber_loss
#poch 100/100 - loss: 0.00010463290619215996
############################# amplitude only #########################
#epoch 100/100 - loss: 7.008184415389543e-05
######################## frequency only ############################
#epoch 100/100 - loss: 5.470156481055847e-06
################################# phase only ###############################
#epoch 100/100 - loss: 2.5068276224511632e-05

##### task1 base model base new
#2.7500991103286908e-09
#epoch 82/100 - loss: 1.9940776126340215e-09
# spatial attention
#epoch 81/100 - loss: 1.5707051019264436e-09
#2.538973813174182e-09
#### channel attention
# channel
#epoch 82/100 - loss: 1.975962001188326e-09
#3.0340998103951097e-09

#%% save and load model
# if necessary, saving model to save time later
torch.save(model.state_dict(),"torch_weights_task1_base_CHA.pth")
