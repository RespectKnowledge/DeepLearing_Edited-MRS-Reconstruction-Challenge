# -*- coding: utf-8 -*-
"""
Created on Wed Mar 22 23:00:37 2023
...
path_simulated="D:\\reconsrtcionMRI\\task2\\task2\\track_02_training_data.h5"
### load sample data and limiting to first scan   
with h5py.File(path_simulated) as hf:
    #gt_fids_train = hf["ground_truth_fids"][()]
    ppm = hf["ppm"][()] ### 12*2048
    t= hf["t"][()]  ######12*2048
    #t_trains = hf["target_spectra"][()] ####12*2048     time
    y=hf["target_spectra"][()] #####12*2048 GT
    train_data=hf["transient_fids"][()]  #####12*2048*2*160 ##input signal
...
#################### load model trained on task1 dataset #########
model.load_state_dict(torch.load("torch_weights_task1_base_combinee_new_finallast.pth"))

################################# trained it from scracht #####
#model.train()
# We already know that in order to backpropagate through 
# layers we have to set it to True
######### trained it from sctrach
# for param in model.parameters():
#     param.requires_grad = True

# for name, child in model.named_children():
#     print(name)
########### finetuned_few layer on task2 dataset ###############
for name, child in model.named_children():
    if name in ['end_conv_1_1', 'end_conv_1_2']:
...
#%% training and validation of finetuned layer #################
...
if skip_train:
    ...
    for epoch in range(0,epoch_count):
        ...
        if loss_epo<test_loss:
            torch.save(model.state_dict(),"torch_weights_task2_base_combinee_task2ch_finallastmore.pth")
            test_loss=loss_epo
    ################### hyber loss function ##################
