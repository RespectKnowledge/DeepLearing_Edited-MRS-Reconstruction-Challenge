# -*- coding: utf-8 -*-
"""
Created on Tue Mar 28 01:24:57 2023
...
model = UNET(40).float()
model.load_state_dict(torch.load("torch_weights_task3_base_combinee_2048_finallast.pth"))
model.eval()
#%% testing task3 2048 spectrum
...
############################################# load sample data #######################################
with h5py.File("D:/reconsrtcionMRI/task3/task3/track_03_test_data.h5") as hf:
    ppm=hf['data_2048']["ppm"][()]
    t=hf['data_2048']["t"][()]
    #y=hf['data_2048']["target_spectra"][()]
    noise_fid=hf['data_2048']["transient_fids"][()]#12*2048*2*160 ##input signal
    
# # Transforming time domain noisy transients into frequency domain difference transients
noise_spec_a = np.fft.fftshift(np.fft.ifft(noise_fid,axis=1),axes=1)
...
for i,d in enumerate(train_dataloader):
    ...
    #del pred
    #break
pred_task3_2048=np.array(predf)
#%% ############### save predictions
import h5py
import numpy as np
...
save_submission(pred_task3_2048,ppm_test.numpy(),"track03_invivo_hetrogen_2048.H5")